...
public:
   ...
   *
   * In addition to the passed arguments, the node's anchor is set to the bottom
   * left and it should not be tinted relative to it's parent's color.
   *
   ...
   */
  ~TilemapView() { _node->removeFromParent(); }
...
public:
  ...
  const std::shared_ptr<scene2::PolygonNode> &getNode() const { return _node; }
...
public:
   ...
   */
  void setSize(Size size) { _node->setPolygon(Rect(Vec2(0, 0), size)); }
};
