...
class LevelModel : public Asset {
    ...
    std::vector<std::vector<Vec2>> _portraits;
...
#pragma mark Static Constructors
     ...
     *
     * The source file can be set at any time via the setFile() method. This
     * method does NOT load the asset.  You must call the load() method to do
     * that.
     *
     ...
     *
     * This method does NOT load the level. You must call the load() method to
     * do that. This method returns false if file does not exist.
     *
     * @return  an autoreleased level file
     */
...
#pragma mark Getters
    ...
    std::vector<std::vector<Vec2>> getPortaits() { return _portraits; }
...
#pragma mark Asset Loading
     ...
     * in parallel, not in sequence.  If an asset (like a game level) has
     * references to other assets, then these should be connected later, during
    ...
    /**
     * Loads this game level from a JsonValue containing all data from a source
     * Json file.
     *
     * This load method should NEVER access the AssetManager.  Assets are loaded
     * in parallel, not in sequence.  If an asset (like a game level) has
     ...
     * scene initialization.
     *
     * @param json the json loaded from the source file to use when loading this
     * game level
     *
     ...
     *
     * This load method should NEVER access the AssetManager.  Assets are loaded
     * and unloaded in parallel, not in sequence.  If an asset (like a game
     * level) has references to other assets, then these should be disconnected
     * earlier.
     */
    void unload();
