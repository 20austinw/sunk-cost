...
public:
    ...
    Minimap(const std::shared_ptr<cugl::AssetManager>& assets, const std::shared_ptr<Scene2>& scene, const std::shared_ptr<TilemapController>& tilemap) :_assets(assets), _scene(scene), _tilemap(tilemap) {
        _minHeight = 300;
        _heightMargin = 100;
        _widthMargin = 100;
        ...
        if(_maxWidth/_maxHeight > _node->getSize().width/_node->getSize().height) {
            // Use height scaling
