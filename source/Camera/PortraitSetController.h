...
public:
   ...
   */
  void addPortrait(int id, Vec3 position, Vec3 direction, Vec2 directionLimits,
                   int battery = 600, int type = 2) {
    _portraits.push_back(
        makePortrait(id, position, direction, directionLimits, battery, type));
  }
...
#pragma mark Main Functions
  void setIndex(int index) { _model->setIndex(index); }
...
public:
   ...
   * @return Rectangle that contains the camera viewpoint
   */
  int getIndex() { return _model->getIndex(); }
...
public:
  ...
  /**
   * Updates camera direction limits. [d.x,d.y] should be the range of possible
   * values for angle.
   */
   ...
   */
  void rotate(float angle) { _portraits[_model->getIndex()]->rotate(angle); }
  ...
  void updateBatteryNode() {
    ...
    Vec2 pos =
        _portraits[_model->getIndex()]->getPosition() + Vec2(offset.size);
  ...
  bool getCurState() { return _portraits[_model->getIndex()]->getState(); }
