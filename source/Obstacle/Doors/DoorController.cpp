...
bool DoorController::update(bool started, bool released, Vec2 touchPos) {
    // detect a drug action to enable the animation for an unlocked door, then
    // update step for the frame number
    if (started && _model->getState() == 0) {
        _model->setState(2);
        _model->setStep(abs(touchPos.distance(_model->getPosition())));
    }
    // detect if ending pos for the drug is on an unlocked door, change to
    // locked
...
void DoorController::updateFrame(Vec2 pos) {
    ...
    if ((frame >= 0) &&
        ((_model->getType() == 0 && frame <= FRAME_NUM_FRONT - 1) ||
...
bool DoorController::isInBound(Vec2 pos) {
    ...
    if (dist <= _view->getSize().width / 2 &&
        dist <= _view->getSize().height / 2) {
        return true;
    }
...
Vec2 DoorController::getModelPosition() { return _model->getPosition(); }

Vec2 DoorController::getViewPosition() { return _view->getPosition(); }

bool DoorController::isLocked() { return _model->getState() == 1; }
