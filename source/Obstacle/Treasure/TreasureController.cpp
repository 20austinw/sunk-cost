...
TreasureController::TreasureController(
    ...
    Vec2 playerSize, Vec2 position) {
#define TREASURE_SIZERATIO 25
    _model = std::make_shared<TreasureModel>(position, 1000);
    _model->setPosition(position);
    _view = std::make_unique<TreasureView>(assets, position,
                                           playerSize * TREASURE_SIZERATIO);
...
void TreasureController::setAsObstacle(
 ...
 */
Vec2 TreasureController::getPosition() { return _model->getPosition(); }

b2Body* TreasureController::getTreasureBody() { return _model->getBody(); }
...
bool TreasureController::getTrigger() { return _model->getTrigger(); }
 ...
 */
void TreasureController::addChildTo(
 ...
 */
void TreasureController::removeChildFrom(
    const std::shared_ptr<cugl::Scene2>& scene) {
    scene->removeChild(_view->getNode());
}

bool TreasureController::update() { return _model->update(); }

std::shared_ptr<TreasureModel> TreasureController::getModel() { return _model; }
